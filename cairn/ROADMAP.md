...
_Last hygiene check: 2026-09-15, M003 done and archived, two review findings added as candidate rows, budgets within caps_

## Milestones

| ID | Title | Status | Depends on | Priority | File/Archive |
|---|---|---|---|---|---|
| M003 | Time binning | done | — | high | milestones/archive/M003-time-binning.md |
...
- Input check for duplicate (id, time) rows, which the stat now pools without notice (M002 review finding 9), and for rows with a value but an NA time, which `bin_core()` drops without notice (M003 review finding 3) — added 2026-09-15, extended 2026-09-15 — DESIGN input contract
- With `bin` set, a `group` coarser than the series id (for example a constant) averages every series together and draws zero-width bands with no warning; only the no-group sentinel is caught (M003 review finding 2) — added 2026-09-15 — R/stat-chromodoris.R setup_params
